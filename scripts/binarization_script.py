...
def train_binary_unet(name=None, seed=None, train_val_split_rate=0.8):
    seed = seed
    batch_size_train = 1
    target_size = (256, 256)
    ...
    unet_checkpoint_filepath = "E:/ED_MS/Semester_3/Codes/MyProject/checkpoints/vanilla_unet.h5"
    binary_unet_checkpoint_filepath = "E:/ED_MS/Semester_3/Codes/MyProject/checkpoints/binary_unet.h5"
    # *: continue training
    start_epoch = 0
    end_epoch = 250
    ...
    model_checkpoint_callback = keras.callbacks.ModelCheckpoint(filepath=binary_unet_checkpoint_filepath,
                                                                ...
                                                                save_best_only=True)
    # *: lr scheduler
    lr_scheduler_callback = keras.callbacks.LearningRateScheduler(schedule=get_lr_scheduler(120))
    tensorboard_val_image_writer = tf.summary.create_file_writer(logdir + "/val_image")
    tensorboard_callback = callbacks.TensorBoard(log_dir=logdir)
    validation_plot_callback = get_validation_plot_callback(binary_unet, data_gen_val, [0, 1],
...
if __name__ == '__main__':
    train_binary_unet(name="binary_unet-transfer_learning", seed=1, train_val_split_rate=0.8)
